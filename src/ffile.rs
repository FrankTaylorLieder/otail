...
impl FFile {
    ...
    async fn set_filter_state(&mut self, filter_state: Option<FilterState>) -> Result<()> {
        ...
        for (client_id, client) in self.clients.iter() {
            ...
            trace!(
                "Clear command sent successfully to client: id={}",
                client_id
            );
        }
    ...
    async fn handle_client_command(&mut self, cmd: FileReq<FFResp>) -> Result<()> {
        match cmd {
            ...
            FileReq::RegisterClient { id, client_sender } => {
                ...
                trace!(
                    "Sending initial stats to new filter client: id={}, stats=0/0",
                    id
                );
                client_sender
                    ...
                    .await?;
                trace!(
                    "Initial stats sent successfully to new filter client: id={}",
                    id
                );
    ...
    async fn next_spooling(
        ...
        self.if_req_sender
            ...
            .await?;
        trace!(
            "Continued spooling GetLine request sent successfully: id={}, line_no={}",
            self.id,
            filter_state.next_line_to_request
        );
                ...
                if line_no < filter_state.next_line_expected {
                    ...
                    for (id, client) in self.clients.iter() {
                        ...
                        client
                            ...
                            .await?;
                        trace!(
                            "Requested filter line sent successfully to client: id={}, match_no={}",
                            id,
                            match_no
                        );
                    }
